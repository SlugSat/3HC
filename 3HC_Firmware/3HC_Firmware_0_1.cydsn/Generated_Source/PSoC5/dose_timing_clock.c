...
/*******************************************************************************
* Function Name: dose_timing_clock_Start
********************************************************************************
*
* Summary:
...
void dose_timing_clock_Start(void) 
{
    ...
    dose_timing_clock_CLKEN |= dose_timing_clock_CLKEN_MASK;
	dose_timing_clock_CLKSTBY |= dose_timing_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: dose_timing_clock_Stop
********************************************************************************
*
* Summary:
...
void dose_timing_clock_Stop(void) 
{
    ...
    dose_timing_clock_CLKEN &= (uint8)(~dose_timing_clock_CLKEN_MASK);
	dose_timing_clock_CLKSTBY &= (uint8)(~dose_timing_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: dose_timing_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((dose_timing_clock_CLKEN & dose_timing_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = dose_timing_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = dose_timing_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* dose_timing_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(dose_timing_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(dose_timing_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: dose_timing_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: dose_timing_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = dose_timing_clock_GetSourceRegister();
    uint16 oldDivider = dose_timing_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = dose_timing_clock_CLKEN & dose_timing_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(dose_timing_clock_DIV_PTR, clkDivider);
                dose_timing_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                dose_timing_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(dose_timing_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((dose_timing_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(dose_timing_clock_DIV_PTR, clkDivider);
				dose_timing_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: dose_timing_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(dose_timing_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: dose_timing_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    dose_timing_clock_MOD_SRC |= modeBitMask & (uint8)dose_timing_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: dose_timing_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    dose_timing_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(dose_timing_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: dose_timing_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return dose_timing_clock_MOD_SRC & (uint8)(dose_timing_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: dose_timing_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = dose_timing_clock_GetDividerRegister();
    uint8 oldSrc = dose_timing_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        dose_timing_clock_MOD_SRC |= CYCLK_SSS;
        dose_timing_clock_MOD_SRC =
            (dose_timing_clock_MOD_SRC & (uint8)(~dose_timing_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: dose_timing_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return dose_timing_clock_MOD_SRC & dose_timing_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: dose_timing_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    dose_timing_clock_PHASE = clkPhase & dose_timing_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: dose_timing_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return dose_timing_clock_PHASE & dose_timing_clock_PHASE_MASK;
}
